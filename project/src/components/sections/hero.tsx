...
export default function Hero() {
  ...
  <div className="absolute inset-0 bg-white dark:bg-black/20" />
  <div className="absolute inset-0">
    <div className="hidden dark:block bg-[url('/images/GroupPhoto.JPG')] bg-cover bg-center opacity-90 h-full w-full transition-all group-hover:blur-sm" />
    <div className="dark:hidden block bg-[url('/images/GroupPhoto.JPG')] bg-cover bg-center opacity-100 h-full w-full transition-all duration-300 group-hover:blur-sm" />
  </div>
      <div className="container relative mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center text-center group">
        <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-6">
        <span className="text-[#0326f3]">
          Connect. Engage. Inspire.
        </span>
        </h1>
        <p className="text-xl md:text-2xl max-w-3xl mb-8 text-black">
          Join our vibrant community and discover amazing events that bring people together.
        </p>
        <br /><br />
        <div className="flex flex-col sm:flex-row gap-4">
          <Button 
            ...
            onClick={() => document.getElementById('join-now')?.scrollIntoView({ behavior: 'smooth' })}
          >
            Join Us
          </Button>
          <Button 
